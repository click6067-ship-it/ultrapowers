...
```
/kickoff (다중백본 Plan Council → sealed/spec-seed.json) ← "방향" 소유
   ↓
...
**최소 충분 산출물 규칙:** 문서의 각 섹션은 `결정 하나를 바꾸거나`, `테스트 가능성을 만들거나`, `실패 위험을 줄여야` 한다. 셋 중 하나도 못 하면 삭제하거나 링크로 이동한다. “1페이지”는 기본 지향점이지 고스테이크·규제·복잡 데이터 설계의 하드 상한이 아니다.
...
### 0. 입력·스테이크 판정
- kickoff 산출 **`sealed/spec-seed.json`**(v3 정본; 구 run은 `plan.md`)이 있으면 그걸 씨앗으로. 없고 요구가 모호하면 **Phase 0 Gate(Intake 재질의)부터** — specpack은 정식화 도구지 정의 도구가 아니다.
