...
The per-theme axis values live as comments at the top of each theme's tokens block (업스트림 repo의 `site/css/tokens.css` — 이 설치본에는 미포함). When in doubt, name your candidate theme out loud and identify its three axis values; if two of three match the previous output, redirect.
