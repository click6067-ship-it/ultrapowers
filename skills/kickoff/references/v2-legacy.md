...
**+ BLOCKER 재질의 형식 (2026-07-24 maintainer 지시 — 대화가 아니라 선택, 단 사고를 가두지 말 것):** BLOCKER 질문은 서술형 대화로 묻지 않는다 — **`AskUserQuestion`(항목선택형)으로, 질문당 선택지 2~4개 + 권장안에 "(Recommended)" 표시.** 사용자가 고르기만 하면 되게. 한 번에 최대 4질문 묶어서. 8칸 중 "미정" 칸이 BLOCKER면 그 칸을 채우는 선택지를 제시한다.
...
**+ Prior-art 피벗 게이트 (2026-07-24 maintainer 지시 — 리서치가 방향을 흔들면 휴먼루프):** Prior-art/리서치(경쟁사·레퍼런스·RISK 검증) 결과가 **피벗 신호**면 — 기존 제품이 이미 문제를 해결 / flagged-assumption의 kill 조건 발동 / 시장·기술 현실이 전제와 다름 — 모델끼리 조용히 피벗하거나 원안을 강행하지 않는다. **발견 요약 + 옵션(원안 유지 / 피벗안 / 중단)을 `AskUserQuestion`(선택형)으로 사용자에게 되묻는다.** 방향 결정은 사용자 소유 — 리서치 후 재질의 루프가 한 번 더 도는 게 정상 경로다.
