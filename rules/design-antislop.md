...
**문제**: LLM 디자인은 학습분포 최빈값(Inter·보라 그라데이션·SaaS 카드 템플릿)으로 수렴한다 — 프로세스가 프롬프트를 이긴다.

**⛔ 예외 — kickoff EXPLORE lane:** 아래 "레퍼런스 먼저"는 **`/kickoff` 발산 단계에 적용되지 않는다.** Plan A·C는 봉인 전 레퍼런스를 보지 않는 것이 설계이고(`rules/gate-loop.md`), 여기서 강제하면 3-lane blind 발산이 무효화된다. Council 중 레퍼런스는 **Plan B lane에만**, 이 파일의 강제 순서는 ADOPT된 **구현 단계부터**.
