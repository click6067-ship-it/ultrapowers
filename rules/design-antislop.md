...
**⛔ 예외 — kickoff EXPLORE lane (2026-07-29 감사 BLOCKER):** 아래 "레퍼런스 먼저"는 **`/kickoff` 발산 단계에 적용되지 않는다.** Plan A(first-principles)와 C(distant-analogy)는 봉인 전 레퍼런스를 보지 않는 것이 설계이고(`rules/phase0-gate.md`), 여기서 레퍼런스를 강제하면 3-lane blind 발산이 통째로 무효화된다. Council 중에는 레퍼런스를 **Plan B lane에만** 주고, 이 파일의 강제 순서는 방향이 ADOPT된 **구현 단계부터** 적용한다.
